...
Most opcodes now implemented.  Just `RTI` left.  In order to finish those, I'll need to implement proper interrupts (hardware `IRQ` done, still `NMI` TODO) so that they can be returned from.
